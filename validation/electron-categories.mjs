...
try {
  ...
  await wait(`!document.getElementById('category-section').hidden`);
  assert.equal(await js(`document.getElementById('draft-category').value`),'');
  ...
  assert.equal(await js(`document.getElementById('draft-category').options.length`),6);
  ...
  await select('');assert.equal((await saved()).category,undefined);
  await win.reload();await wait(`!document.getElementById('category-section').hidden`);assert.equal(await js(`document.getElementById('draft-category').value`),'');
  const other=await createJob('다른 글');
  await select('11');await saved();
  await js(`localStorage.setItem('hdev.current',${JSON.stringify(other.id)});location.reload()`);await wait(`!document.getElementById('category-section').hidden`);
  assert.equal(await js(`document.getElementById('draft-category').value`),'');
