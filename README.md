...
[H.Dev Studio 0.7.5 다운로드](https://github.com/Hlxecz/blog-automation/releases/tag/v0.7.5)에서 `HDev-Studio-0.7.5-win-x64.exe`를 받아 실행하세요. 설치가 필요 없는 Windows 64비트 앱입니다.
...
데스크톱 앱에서는 티스토리 로그인 후 내 블로그를 선택합니다. 웹 모드에서는 생성된 `tistory.config.json`의 `blogUrl` 또는 앱의 설정 화면에 주소를 입력합니다. 0.7.5부터 사용자 요청으로 H.Dev Log의 6,408자 말투 프로필을 기본 지침에 포함합니다. **내 말투 → 기본 블로그용 프롬프트**에서 확인하고 수정할 수 있습니다. `setup`을 다시 실행해도 기존 설정은 보존합니다.
...
배포 EXE의 기본 블로그 주소는 미설정 상태이며 로그인한 계정에서 블로그를 선택합니다. 기본 프롬프트에 등장하는 H.Dev Log의 글 주소는 문체 참고자료이며 로그인·발행 대상 설정이 아닙니다. 0.7.5는 이전의 짧은 기본 지침을 수정 없이 쓰는 경우에만 백업 후 새 기본 지침으로 교체합니다. 사용자가 수정한 프롬프트와 카테고리별 지침은 유지합니다.
