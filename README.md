...
[H.Dev Studio 0.7.4 다운로드](https://github.com/Hlxecz/blog-automation/releases/tag/v0.7.4)에서 `HDev-Studio-0.7.4-win-x64.exe`를 받아 실행하세요. 설치가 필요 없는 Windows 64비트 앱입니다.
...
데스크톱 앱에서는 티스토리 로그인 후 내 블로그를 선택합니다. 웹 모드에서는 생성된 `tistory.config.json`의 `blogUrl` 또는 앱의 설정 화면에 주소를 입력합니다. `style/profile.md`에 말투 지침을 준비하세요. 기본 템플릿은 특정 사용자의 글을 분석한 프로필이 아닙니다. `setup`을 다시 실행해도 기존 설정은 보존합니다.
