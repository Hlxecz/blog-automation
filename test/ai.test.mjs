...
if(prompt.includes('가상 기록')){const references=JSON.parse(prompt.split('<references>')[1].split('</references>')[0]);assert.equal(references[0].text,'참고한 문서 본문');assert.equal(references[0].url,'https://example.com/docs');assert.equal(references[0].status,'read');const writing=JSON.parse(prompt.split('<category_writing_preferences>')[1].split('</category_writing_preferences>')[0]);assert.equal(writing.category.id,'11');assert.equal(writing.prompt,'핵심 소식부터 설명합니다.');}
...
test('both CLI protocols receive actual image bytes/paths and return large Korean structured output and style results', async t => {
  ...
  for (const provider of ['codex', 'claude']) {
    ...
    const result = await generate({ root, directory: root, title: '가상 기록', notes: '자료', style: '짧게 작성', provider, writing:{category:{blogUrl:'https://example.tistory.com',id:'11',path:['AI','뉴스']},prompt:'핵심 소식부터 설명합니다.'}, references:[{url:'https://example.com/docs',text:'참고한 문서 본문',status:'read'}] });
