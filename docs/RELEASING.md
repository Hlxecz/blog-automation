...
0.4.0부터 빌드에는 `config/`의 공유용 기본값을 포함합니다. 0.8.0는 사용자 요청에 따라 H.Dev Log 말투 프로필 6,408자 사본을 `config/style.example.md`에 포함합니다. 로컬 `tistory.config.json`, 말투 프로필·참고 원문, AI 선택값, 사진·초안·로그인 자료는 빌드 대상에 포함하지 않습니다. 처음 실행한 PC에 기본 설정을 만들며, 기존 PC에서는 수정하지 않은 구버전 기본 지침만 백업 후 교체하고 사용자가 수정한 말투·기존 설정은 유지합니다.
...
커밋 제목은 **`타입: 한국어 설명`**으로 작성합니다. 타입은 아래 영문 표기를 사용하고, 제목의 설명과 본문은 한국어로 씁니다. 제품명·기술명·파일명은 원래 표기를 유지할 수 있습니다.
...
| `fix` | 버그 수정 |
| `docs` | 문서 수정 |
| `style` | 기능 변경 없는 코드 서식 수정 |
| `refactor` | 동작을 유지하는 코드 구조 개선 |
| `test` | 테스트 추가·수정 |
| `chore` | 빌드 설정, 의존성, 자동 검사 설정 등 관리 작업 |

- 한 커밋에는 하나의 목적을 담습니다. 서로 독립적인 기능·수정은 나눕니다.
- 실제로 무엇이 바뀌었는지 짧고 구체적으로 설명합니다. `코드 수정`, `버그 수정`처럼 대상이 불분명한 제목은 피합니다.
- 본문이 필요하면 제목 다음에 빈 줄을 두고 이유·변경 내용·검증을 한국어로 적습니다.
- 과거 소스를 배포본에서 복원한 작업은 실제 기능 개발 시점으로 오해하지 않도록 `chore: … 소스 복원`으로 표시합니다.

```text
feat: 칸별 글 다듬기 기능 추가
fix: 로그인한 계정의 운영 블로그로 연결
docs: 새 PC 설치 가이드 추가
refactor: 카테고리 검증 로직 분리
test: 글 다듬기 실패 시 원문 보존 검사 추가
chore: 빌드 시 카테고리 화면 검사 실행
```
...
사진·초안·개인 설정·로컬 말투 원문·로그·EXE가 포함되지 않았는지 확인한 뒤 실행합니다. 사용자가 공유를 요청한 `config/style.example.md` 사본은 배포 대상입니다. 커밋 메시지는 실제 변경 내용을 적습니다.

```powershell
git commit -m "feat: 칸별 글 다듬기 기능 추가"
...
- **Windows checks:** `main`에 push하거나 PR을 만들면 설정 초기화, 버전 검사, Node 테스트와 Electron 편집기·카테고리·글쓰기 프롬프트 화면 검증을 실행합니다. 모의 목록과 임시 자료 폴더에서 선택 보관·재생성 시 유지·목록 읽기 실패 복구·카테고리 지침 적용·기본 프롬프트 보존·모바일 배치를 확인합니다.
