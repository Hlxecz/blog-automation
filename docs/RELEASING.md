...
0.4.0부터 빌드에는 `config/`의 공용 템플릿만 포함합니다. 로컬 `tistory.config.json`, 말투 프로필·참고 원문, AI 선택값, 사진·초안·로그인 자료는 빌드 대상에 포함하지 않습니다. 처음 실행한 PC에 공용 설정을 만들며, 이미 자료가 있는 PC에서는 기존 설정과 말투를 유지합니다.
