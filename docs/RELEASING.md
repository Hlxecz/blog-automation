...
0.4.0부터 빌드에는 `config/`의 공유용 기본값을 포함합니다. 0.7.5는 사용자 요청에 따라 H.Dev Log 말투 프로필 6,408자 사본을 `config/style.example.md`에 포함합니다. 로컬 `tistory.config.json`, 말투 프로필·참고 원문, AI 선택값, 사진·초안·로그인 자료는 빌드 대상에 포함하지 않습니다. 처음 실행한 PC에 기본 설정을 만들며, 기존 PC에서는 수정하지 않은 구버전 기본 지침만 백업 후 교체하고 사용자가 수정한 말투·기존 설정은 유지합니다.
